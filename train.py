...
def parse_args():
    ...
    #parser.add_argument('--config', default='./configs/abcnet/aabcnet_clft-l_256x256_1500e_nudt.py', help='train config file path')
    parser.add_argument(
...
class Train(object):
    ...
    def training(self, epoch, ifprint):
        self.model.train()
        losses = []
        if args.local_rank != -1:
        ...
        for i, (img, mask, edge) in enumerate(self.train_data):
            since = time.time()
            if args.local_rank != -1:
                img = img.cuda()
                mask = mask.cuda()
            ...
            preds = self.model(img, edge, epoch, ifprint=False)
    ...
    def testing(self, epoch, ifprint):
        self.model.eval()
        ...
        with torch.no_grad():
            for i, (img, mask, edge) in enumerate(self.test_data):
                if args.local_rank != -1:
                    img = img.cuda()
                    mask = mask.cuda()
                    edge = edge.cuda()
                    ...
                    mask = mask.to(self.device)
                    edge = edge.to(self.device)
                preds = self.model(img, edge, epoch, ifprint=True)
                if self.deep_supervision and self.cfg.model['decode_head']['deep_supervision']:
                    loss = []
                    for pre in preds:
                        loss.append(self.criterion(pre, mask))
...
def main(args):
    ...
    for i in range(start, cfg.runner['max_epochs'] + 1):
        trainer.training(i,ifprint=False)
        trainer.testing(i,ifprint=True)
