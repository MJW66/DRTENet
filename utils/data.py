...
class DatasetLoad(Data.Dataset):
    ...
    def _sync_transform(self, img, mask, edge):
        if self.mode == 'train' and self.data_aug:
            ...
            img, mask, edge = np.array(img), np.array(mask), np.array(edge)
            ...
            edge = edge.resize((self.base_size, self.base_size), Image.BILINEAR)
            img, mask, edge = np.array(img), np.array(mask), np.array(edge)
            img = self.transform(img)
            mask = transforms.ToTensor()(mask)
            edge = transforms.ToTensor()(edge)
    ...
    def __getitem__(self, item):
        ...
        edge_path = osp.join(self.data_dir, 'edges', img_name)  # ----------------------------
        img = Image.open(img_path).convert('RGB')
        mask = Image.open(label_path)
        edge = Image.open(edge_path).convert('RGB')
        img, mask, edge = self._sync_transform(img, mask, edge)
        return img, mask, edge
